...
public class NomadicWalkEntity extends TabuEntity {
    ...
    Boolean nomadic = false; //拠点を移動するかどうか
    ...
    public void step() {
        ...
        if(homeSick){
            ...
            //System.out.printf("%d entity is homesick\n", current_entity);
            if( this.random.nextInt(100) < gamma) { //gamma%の確率で次回homeを移動
                nomadic = true;
            }
            return;
        }
        ...
        levyWalkStep();
        if(this.random.nextInt(100) < alpha) { //alphaの確率でhomeSickに移行
            homeSick = true;
        }
        if(nomadic) { //nomadicならhomeを移動し,nomadic終了
        ...
        for (Integer i = 1; i <= hopLength; i++) // hopLength分のLevyWalkを繰り返す
            {
                ...
                if (i >= hopLength) {
                    ...
                    if(current_entity.equals(entityNum -1)) {
                        //System.out.printf("%d entity call checkmeet\n", current_entity);
                        CheckMeetEntity();
                        return;
                    }
                }
    ...
    public void setRandomSeed(Long seed) {
        if (seed.equals(0L)) {
            this.random.setSeed(new SecureRandom().nextLong());

        } else {
            this.random.setSeed(seed);
        }
        this.random.nextInt(100);//一度目は偏りやすいので消す
        return;
    }
        ...
        for (Integer i = 0; i < entityNum; i++) {
            for (Integer compare_entity = 0; compare_entity < entityNum; compare_entity++) {
                ...
                if (nodeID[compare_entity].equals(nodeID[i])) {
                    ...
                    RandomWalkOnGraph.encountCount.get(i).set(compare_entity, RandomWalkOnGraph.encountCount.get(i).get(compare_entity) + 1);
                }
            }
        }
