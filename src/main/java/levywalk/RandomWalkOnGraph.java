...
public class RandomWalkOnGraph extends RandomWalk {
    ...
    final Integer alpha; // パラメータ
    final Integer gamma; // パラメータ
    final Long graphSeed; // シード
    final Integer interval; // カバー率を取得するステップの間隔
    final Boolean debug;
    Long walkSeed; // シード

    static HashMap<Integer, ArrayList<Integer>> encountCount = new HashMap<>(); // 各エンティティの遭遇記録
    ...
    public RandomWalkOnGraph(Data d) {
        ...
        } else if (d.entityClass.equals("NomadicWalk")) {
            if (this.walkSeed.equals(0L))
                this.walkSeed = new Random().nextLong();
            this.entityClass = NomadicWalkEntity.class.getName(); // NomadicWalk

        } else if (d.entityClass.equals("HomesickWalk")) {
            if (this.walkSeed.equals(0L))
                this.walkSeed = new Random().nextLong();
            this.entityClass = HomesickWalkEntity.class.getName(); // Homesick LevyWalk
    ...
    public void init(Graph graph) {
        ...
        NomadicWalkEntity.border_entity = entityNum / 2;

        // 各ノードへの最短経路の計算
        /* ダイクストラ法で求める場合 */
        for (Node n : graph) {
            Dijkstra dijk = new Dijkstra(Dijkstra.Element.EDGE, null, null);
            dijk.init(graph);
            dijk.setSource(n);
            dijk.compute();
            NomadicWalkEntity.dijkstra.put(Integer.valueOf(n.getId()), dijk);
            HomesickWalkEntity.dijkstra.put(Integer.valueOf(n.getId()), dijk);
        }

        /****************************/

        /* ワーシャルフロイド法で求める場合(バグあり) */
        /*
         * APSP apsp = new APSP(graph, null, false); //graphについて、重み・エッジの向きを考慮しないAPSPを作成
         * apsp.compute(); //最短経路の計算
         * 
         * /
         ****************************/
            ...
            if (anEntity instanceof LevyWalkEntity) {
                ...
                if (this.current_trial == 0 && entitiy_count == 0) {
                    WriteToFile(write_file,
                            "lamdaA: " + this.lambda[entitiy_count] + ", lamdaB: " + this.lambda[this.entityNum
                                    / 2] + "\n");
                    WriteToFile(write_file, "0\n");
                }
                */
                lwEntity.setRandomSeed(this.walkSeed); // シードの設定
            } else if (anEntity instanceof NomadicWalkEntity) {
                NomadicWalkEntity nEntity = (NomadicWalkEntity) anEntity; // ダウンキャスト
                nEntity.graph = graph;
                nEntity.entityNum = entityNum;
                nEntity.researchCoverRatio = this.researchCoverRatio;
                nEntity.permissibleError = this.permissibleError;
                nEntity.lambda = this.lambda;
                nEntity.alpha = this.alpha;
                nEntity.gamma = this.gamma;
                nEntity.debug = this.debug;
                nEntity.file = this.file;
                nEntity.current_entity = entitiy_count;
                /*二つの集団の遭遇率は計測しないので必要ない
                String file_name = "meet_result/result_meet_count";
                File write_file = new File((file_name.endsWith(".csv")) ? file_name : file_name.concat(".csv"));
                nEntity.write_file = write_file;
                if (this.current_trial == 0 && entitiy_count == 0) {
                    WriteToFile(write_file,
                            "lamdaA: " + this.lambda[entitiy_count] + ", lamdaB: " + this.lambda[this.entityNum
                                    / 2] + "\n");
                ...
                */
                nEntity.setRandomSeed(this.walkSeed); // シードの設定

            } else if (anEntity instanceof HomesickWalkEntity) {
                HomesickWalkEntity hEntity = (HomesickWalkEntity) anEntity; // ダウンキャスト
                hEntity.graph = graph;
                hEntity.entityNum = entityNum;
                hEntity.researchCoverRatio = this.researchCoverRatio;
                hEntity.permissibleError = this.permissibleError;
                hEntity.lambda = this.lambda;
                hEntity.alpha = this.alpha;
                hEntity.debug = this.debug;
                hEntity.file = this.file;
                hEntity.current_entity = entitiy_count;
                /*二つの集団の遭遇率は計測しないので必要ない
                String file_name = "meet_result/result_meet_count";
                File write_file = new File((file_name.endsWith(".csv")) ? file_name : file_name.concat(".csv"));
                hEntity.write_file = write_file;
                if (this.current_trial == 0 && entitiy_count == 0) {
                    WriteToFile(write_file,
                            "lamdaA: " + this.lambda[entitiy_count] + ", lamdaB: " + this.lambda[this.entityNum
                                    / 2] + "\n");
                ...
                */
                hEntity.setRandomSeed(this.walkSeed); // シードの設定
        ...
        for (int i = 0; i < entityNum; i++) { // 各エンティティの遭遇記録を初期化
            encountCount.put(i, new ArrayList<Integer>());
            for (int j = 0; j < entityNum; j++) {
                encountCount.get(i).add(0);
            }
    ...
    public void coloring(Graph graph) {
        ...
        for (Node aNode : graph.getEachNode()) {
            if (aNode.hasAttribute("start")) { // 初期位置
                aNode.addAttribute("ui.style", "fill-color: orange; size: 12px;"); // オレンジ
                if (this.getPasses(aNode) > 0) { // なおかつ、通過している
                    // aNode.addAttribute("ui.style", "fill-color: orange; size: 6px;"); // オレンジ
                }
            } else if (aNode.hasAttribute("home")) { // 拠点になったことがある
                // aNode.addAttribute("ui.style", "fill-color: orange; size: 9px;"); // オレンジ
    ...
    private void writeMeetResult() {
        try {
            FileWriter fw = new FileWriter(file, true);
            ArrayList result = new ArrayList<Integer>();
            for (int i = 0; i < entityNum; i++) {
                result = encountCount.get(i);
                fw.write(i + ",");
                for (int j = 0; j < entityNum; j++) {
    ...
    private void getCoverRatio(Graph graph) {
        ...
        System.out.println("");
        System.out.printf("Node: %5.0f / %5.0f, Ratio: %6.3f\n", nodeNumber, allNodeCount,
                nodeNumber / allNodeCount * 100);
        System.out.printf("Edge: %5.0f / %5.0f, Ratio: %6.3f\n", edgeNumber, allEdgeCount,
                edgeNumber / allEdgeCount * 100);
        System.out.println("");
